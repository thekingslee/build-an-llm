...
def sft_train(cfg=None, hf_dataset: str = None, hf_subset: str = None,
    ...
    # MPS does not support AMP — disable automatically
    device = cfg.DEVICE
    ...
    # ------------------- Loss -------------------
    # ignore_index=-100 skips prompt tokens and padding in y automatically
    ...
    for epoch in range(start_epoch, cfg.EPOCHS):
        ...
        for batch_idx, (x, y) in enumerate(
            ...
            with autocast(device_type=device if device != "mps" else "cpu", enabled=use_amp):
                logits = model(x)                               # (N, seq_len, vocab)
                loss = criterion(
                    logits.view(-1, logits.size(-1)),           # (N*seq_len, vocab)
                    y.view(-1),                                 # (N*seq_len,)
                )
